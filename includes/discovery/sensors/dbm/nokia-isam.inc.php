...
foreach ($pre_cache['nokiaIsamSfpPort'] as $slotId => $slot) {
    ...
    foreach ($slot as $portId => $port) {
        ...
        if (is_numeric($port['sfpDiagTxPower'])) {
            ...
            $limit_low = ($port['sfpDiagRSSITxPowerAlmLow'] / $divisor) ?: -2;
            $warn_limit_low = ($port['sfpDiagRSSITxPowerWarnLow'] / $divisor) ?: 1;
            $limit = ($port['sfpDiagRSSITxPowerAlmHigh'] / $divisor) ?: 9;
            $warn_limit = ($port['sfpDiagRSSITxPowerWarnHigh'] / $divisor) ?: 7;
